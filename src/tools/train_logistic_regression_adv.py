...
def train_logistic_regression_model_adv(
    ...
    if "model.pt" in os.listdir(args.save_root):
        ...
        eval_dict = eval_logistic_regression_model(model, test_loader, criterion)
        return eval_dict

    print("[logistic regression] The model doen't exist in model path '{}'. "
          "Train a model with new settings.".format(args.save_root))
    ...
    for epoch in range(args.n_epoch):
        ...
        # train the model
        model.train()
        train_dict = train_logistic_regression_model_adv_epoch(
...
def train_logistic_regression_model_adv_epoch(
        model, train_loader, optimizer, criterion,
        adv_step_size, adv_epsilon, adv_n_step,
        adv_bound_max=None, adv_bound_min=None,
        normalize_fn=None, denormalize_fn=None,
    ...
    for i, (batch_X, batch_y) in enumerate(tqdm(train_loader, desc="train", ncols=100)):
        ...
        batch_X_adv = pgd_attack(
            ...
            loss_func=lambda out, gt: criterion(out.squeeze(1), gt.float()),
        ...
        loss = criterion(outputs_adv, batch_y.float())
        loss.backward()
        optimizer.step()

        preds_adv = torch.round(torch.sigmoid(outputs_adv))
        ...
        with torch.no_grad():
            ...
            loss_clean = criterion(outputs_clean, batch_y.float())
            preds_clean = torch.round(torch.sigmoid(outputs_clean))
...
def eval_logistic_regression_model(model, test_loader, criterion):
    device = next(model.parameters()).device
    # evaluate the performance of the model
    model.eval()
    loss_avg, acc_avg = AverageMeter(), AverageMeter()
    with torch.no_grad():
        for step, (batch_X, batch_y) in enumerate(tqdm(test_loader, desc="test ", ncols=100)):
            batch_X, batch_y = batch_X.to(device), batch_y.to(device)
            bs = batch_X.shape[0]
...
def eval_logistic_regression_model_adv(
    ...
    for i, (batch_X, batch_y) in enumerate(tqdm(test_loader, desc="test ", ncols=100)):
        batch_X, batch_y = batch_X.to(device), batch_y.to(device)
        bs = batch_X.shape[0]
        ...
        with torch.no_grad():
            outputs_adv = model(batch_X_adv).squeeze(1)
            loss_adv = criterion(outputs_adv, batch_y.float())
            preds_adv = torch.round(torch.sigmoid(outputs_adv))
            acc_adv = torch.sum(preds_adv == batch_y) / bs

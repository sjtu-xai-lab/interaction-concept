...
def train_logistic_regression_model(args, model, train_loader, test_loader):
    ...
    for epoch in range(args.n_epoch):
        ...
        # train the model
        model.train()
        train_dict = train_logistic_regression_model_epoch(
        ...
        model.eval()
        with torch.no_grad():
            eval_dict = eval_logistic_regression_model(
        ...
        # show loss
        if epoch % 10 == 0 or epoch == args.n_epoch - 1 or time.time() - t_log > 30:
            print('On train set - Epoch: {} \t Loss: {:.6f} \t Acc: {:.6f}'
                  .format(epoch, train_loss, train_acc))
            print('On test set - Epoch: {} \t Loss: {:.6f} \t Acc: {:.6f}'
...
def train_logistic_regression_model_epoch(model, train_loader, optimizer, criterion):
    ...
    for step, (batch_X, batch_y) in enumerate(tqdm(train_loader, desc="training", ncols=100)):
        ...
        acc = torch.sum(preds == batch_y).item() / bs

        train_loss_avg.update(loss.item(), bs)
        train_acc_avg.update(acc, bs)

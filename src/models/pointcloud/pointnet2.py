"""
    Reference: https://github.com/ada-shen/Interpret_quality/blob/master/models/pointnet2.py
"""
...
def farthest_point_sample(xyz, npoint):
    ...
    farthest = torch.zeros(B, dtype=torch.long).to(device)  # fix the first point in FPS
...
class PointNet2(nn.Module):
    def __init__(self, num_classes):
        ...
        self.drop2 = nn.Dropout(0.4)  # official repo uses 0.4
...
def pointnet2(num_classes):
    return PointNet2(num_classes=num_classes)


if __name__ == '__main__':
    net = PointNet2(num_classes=16)
    print("finish initialization")
    point_cloud = torch.randn(10, 3, 2500)
    print(net(point_cloud).shape)
